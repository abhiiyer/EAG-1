...
def generate_pdf_summary(customer, nba, score, ctype, follow_up):
    ...
    lines = [
        ...
        f"Customer Type: {ctype.encode('ascii', 'ignore').decode()}",
        # f"Customer Type: {ctype}",
        #f"Customer Type: {ctype.replace('📉','').replace('🧍','').replace('💼','').replace('🎯','').replace('🧠','').strip()}",
...
def generate_pdf_summary(customer, nba, score, ctype, follow_up):
    ...
    def write_line(text):
        try:
            ...
            pdf.cell(200, 10, txt="** Encoding Error in Line **", ln=True)
